  ...
  void paint(Canvas canvas, Size size){
    ...
    if(_graphicData.gridCount > 0){
      ...
      for(double i = _maxSize / 2; i < _maxSize; i += _gridStep){
        ...
        canvas.drawLine(Offset(_maxSize - i, 0), Offset(_maxSize - i, _maxSize), paintGrid);

        canvas.drawLine(Offset(0, i), Offset(_maxSize, i), paintGrid);
        canvas.drawLine(Offset(0, _maxSize - i), Offset(_maxSize, _maxSize - i), paintGrid);
    ...
    canvas.drawLine(
        ...
        Offset(_maxSize / 2 + _graphicData.displaceX, _maxSize + _graphicData.displaceY),
        paintAxis);
    canvas.drawLine(
        Offset(_maxSize / 2 + _graphicData.displaceX, _graphicData.displaceY),
        Offset(_maxSize / 2 + AXIS_OFFSET + _graphicData.displaceX, AXIS_OFFSET * 2 + _graphicData.displaceY),
        paintAxis);
    canvas.drawLine(
        Offset(_maxSize / 2 + _graphicData.displaceX, _graphicData.displaceY),
        Offset(_maxSize / 2 - AXIS_OFFSET + _graphicData.displaceX, AXIS_OFFSET * 2 + _graphicData.displaceY),
    ...
    canvas.drawLine(
        ...
        Offset(_maxSize + _graphicData.displaceX, _maxSize / 2 + _graphicData.displaceY),
        paintAxis);
    canvas.drawLine(
        Offset(_maxSize + _graphicData.displaceX, _maxSize / 2 + _graphicData.displaceY),
